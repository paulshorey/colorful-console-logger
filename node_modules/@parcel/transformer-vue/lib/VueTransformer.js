...
var _default = new (_plugin().Transformer)({
  ...
  canReuseAST({
    ...
    return ast.type === 'vue' && _semver().default.satisfies(ast.version, '3.0.0-beta.20');
  },
  ...
  async transform({
    ...
    let out = script == null ? 'let script = {};\n' : `import script from 'script:./${basePath}';\n`;

    if (template != null) {
      out += `import {render} from 'template:./${basePath}';\n` + 'script.render = render;\n';
    }
...
script.__file = \`${options.mode === 'production' ? basePath : asset.filePath.replace(/\\/g, '/')}\`;
...
async function processPipeline({
  ...
  let consolidate = await options.packageManager.require('consolidate', asset.filePath, {
    autoinstall: false
  } // Would have failed by now if it needed autoinstall
  );
